...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<Object> handleUserNotFoundException(UserNotFoundException ex) {
		ErrorResponse errorResponse = new ErrorResponse("404", "Record Not Found");
		Error error = new Error();
		error.setError(errorResponse);
	...
	public ResponseEntity<Object> handleUserNameExistsException(UserNameExistsException ex) {
		ErrorResponse errorResponse = new ErrorResponse("400", "Email address already exists");
		Error error = new Error();
		error.setError(errorResponse);
	...
	public ResponseEntity<Object> handleInvalidPasswordException(InvalidPasswordException ex) {
		ErrorResponse errorResponse = new ErrorResponse("400", "Invalid password format");
		Error error = new Error();
		error.setError(errorResponse);
		return new ResponseEntity(error, HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<Object> handleInvalidParameterException(InvalidParameterException ex) {
		ErrorResponse errorResponse = new ErrorResponse("400", "Invalid phone number format");
		Error error = new Error();
		error.setError(errorResponse);
		return new ResponseEntity(error, HttpStatus.BAD_REQUEST);
	}
